...
const Energy = () => {
  ...
  const [fromUnit, setFromUnit] = useState(
    convert().possibilities("energy")[0]
  );
  const [toUnit, setToUnit] = useState(convert().possibilities("energy")[2]);
  ...
  return (
    <Card>
      <Header>Energy Converter</Header>
      <InputsCard>
        <ConverterInput
          values={convert().possibilities("energy")}
